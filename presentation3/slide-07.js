                             ...
                             ranges={
                                 [
                                     {loc: [0, 87], title: 'Webpack 简介（一）', note: '使用上下方向键查看代码注解'},
                                     {
                                         loc: [0, 87],
                                         title: 'Webpack 简介（一）',
                                         note: <span>学习 webpack 首先要了解 JavaScript 模块化基础知识，建议首先通过如下链接自学 <a
                                             ...
                                             href="http://www.csdn.net/article/2015-04-30/2824595-Modules-in-ES6"
                                             target="_blank">ES6</a> 的模块化规范</span>
                                     },
                                     {
                                         loc: [0, 87],
                                         title: 'Webpack 简介（一）',
                                         note: <span>作为学习 Vue.js 单文件组件的基础，我们只需要了解到 webpack 是一个<span style={{color: '#7ec699'}}>通用 JavaScript 打包器</span>，其主要特点是<span style={{color: '#7ec699'}}>一切皆模块的思想</span>，能够<span style={{color: '#7ec699'}}>使用加载器将其他资源转换为 JavaScript 模块</span></span>
                                     },

                                     {loc: [0, 5], note: 'index.html 文件'},
                                     {loc: [5, 15], note: 'index.html 文件'},

                                     {loc: [16, 24], note: <span>webpack 打包入口文件 entry.js 文件（这里的注释和 script 标签是为将所有代码写入同一个 Code Slide 而加，请忽略，下同），使用打包命令 <span style={{color: '#7ec699'}}>webpack ./entry.js bundle.js</span> 可将此文件打包生成 bundle.js </span>},
                                     ...
                                     {loc: [73, 82], note: '此数组包含打包生成的模块，作为参数传递给 webpack 引导函数，并且可以在 webpack 引导函数中通过数组索引被模块加载函数调用'},
                                     {loc: [75, 81], note: '0 号模块，即 webpack 打包时的入口模块，打包生成的模块是一个函数，注意该函数只有两个参数'},
                                     {loc: [69, 71], note: '加载入口模块并返回其输出'},

                                     {loc: [0, 87], title: 'Webpack 简介（一）', note: <span>看上去 webpack 和其他模块化系统没有什么差异，但其独特之处在于：<span style={{color: '#7ec699'}}>通过可以将其他资源转换成 JavaScript 的加载器，实现一切皆模块</span></span>},
