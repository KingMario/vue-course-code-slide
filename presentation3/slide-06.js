        ...
        return ( < CodeSlide {...this.props }
                             ...
                             ranges={
                                 [
                                     ...
                                     {loc: [75, 83], note: 'activate 钩子，可以是一个单独的回调函数，或者是包含多个回调函数的数组，通常用于准备数据，防止 FOUC 闪烁'},
                                     {
                                         loc: [76, 79],
                                     ...
                                     {loc: [87, 92], note: '实例数据'},
                                     {loc: [88, 89], note: '当前视图数据，可以绑定到动态组件的 is 特殊属性上'},
                                     {loc: [16, 22], note: '链接的 click 事件绑定到内联语句，用于修改当前视图数据，其中的 href 属性用于保持光标形状为正常的手形，也可以使用 css 实现'},
                                     ...
                                     {loc: [23, 24], note: 'name、date、category 等是传递给组件的 props，组件切换过程中，组件声明了的 props 才会被使用'},

                                     {loc: [89, 90], note: '过渡效果数据'},
                                     {loc: [7, 8], note: <span>引入的<a href="https://github.com/haydenbbickerton/vue-animate" target="_blank">过渡效果样式库</a></span>},
